...
for dp, _, fns in os.walk(DATA_ROOT):
    if "case_splits.json" in fns:
        SPLITS = os.path.join(dp, "case_splits.json")
        break
print(
    f"volumes: {VOL_DIR} ({nv}) | packaged: {PKG_DIR} ({npk}) | splits: {SPLITS}")
...
class VolXrayDS(Dataset):
    def __init__(s, ids, vol_dir, pkg_dir, vol_res=64, img_res=128):
        s.ids = ids
        s.vd = vol_dir
        s.pd = pkg_dir
        s.vr = vol_res
    ...
    def __getitem__(s, i):
        cid = s.ids[i]
        vz = np.load(f"{s.vd}/{cid}.npz")
        pz = np.load(f"{s.pd}/{cid}.npz")
        v = F.avg_pool3d(torch.from_numpy(vz["volume"]).float()[
                         None, None], 128//s.vr)[0]
        im = F.interpolate(torch.from_numpy(pz["images"]).float()[
                           None], size=s.ir, mode="bilinear", align_corners=False)[0]
...
def sample_vol(model, sched, images, poses, svoxel, res, device, seed=0, n_steps=50):
    ...
    ab = sched.abars
    T = sched.n
    steps = list(reversed(range(0, T, max(1, T//n_steps))))
    B = images.shape[0]
    ...
    for i, ts in enumerate(steps):
        ...
        abt = ab[ts]
        eps = (x-torch.sqrt(abt)*x0)/torch.sqrt(1-abt+1e-8)
...
lr_sched = torch.optim.lr_scheduler.CosineAnnealingLR(
    opt, T_max=opt_steps, eta_min=1e-5)
ema = EMA(model, decay=0.999)

model.train()
torch.cuda.reset_peak_memory_stats()
b0 = next(iter(tr))
l0 = vol_loss(model, sched, b0, device)
l0.backward()
opt.zero_grad()
...
def eval_dice_ema(ids, n=6):
    ...
    ema.copy_to(model)
    model.eval()
    dss = VolXrayDS(ids, VOL_DIR, PKG_DIR, vol_res=RES)
    sc = []
...
for ep in range(EPOCHS):
    for i, batch in enumerate(tr):
        ...
        if (i + 1) % ACCUM == 0:
            ...
            ostep += 1
            if ostep % 100 == 0:
                print(ostep, round(loss.item()*ACCUM, 4),
                      f"lr {opt.param_groups[0]['lr']:.2e}", int(time.time()-t0), "s", flush=True)
    if ep % 2 == 0 or ep == EPOCHS-1:
        d = eval_dice_ema(va_ids, n=6)
        print(f"== epoch {ep} EMA val Dice(6) {d:.3f} ==", flush=True)
        if d > best:
            best = d
            torch.save(ema.shadow, f"{CKPT}/vol_best_ema_64.pt")
            print("  saved best(EMA)", round(best, 3), flush=True)
...
model.load_state_dict(torch.load(
    f"{CKPT}/vol_best_ema_64.pt", map_location=device, weights_only=True))
model.eval()
norm = json.load(open(f"{REPO}/data/splits/normalization_stats.json"))
cmean = np.array(norm["coord_mean"])
cstd = np.array(norm["coord_std"])
...
def pred_centerline_mm(cid, res=64, thr=0.3, seed=0):
    vz = np.load(f"{VOL_DIR}/{cid}.npz")
    sv = vz["svoxel"].astype(np.float64)
    pz = np.load(f"{PKG_DIR}/{cid}.npz")
    im = F.interpolate(torch.from_numpy(pz["images"]).float()[
                       None], size=128, mode="bilinear", align_corners=False).to(device)
    ...
    vol = sample_vol(model, sched, im, P, svt, res,
                     device, seed=seed)[0, 0].cpu().numpy()
    idx = np.argwhere(skel3d((vol > thr).astype(np.uint8))
                      > 0).astype(np.float64)
    if len(idx) == 0:
        idx = np.argwhere(vol > thr).astype(np.float64)
...
def gt_centerline_mm(cid):
    ...
    sv = vz["svoxel"].astype(np.float64)
    pz = np.load(f"{PKG_DIR}/{cid}.npz")
    cl = pz["centerline"][pz["centerline_mask"].astype(
        bool)][:, :3].astype(np.float64)
...
ov = {1: [], 2: [], 5: []}
for cid in va_ids:
    pr = pred_centerline_mm(cid)
    gt = gt_centerline_mm(cid)
    ch.append(chamfer_l2(pr, gt))
    for d in (1, 2, 5):
        ov[d].append(overlap_metric(pr, gt, d))
print(
    f"\nStage-2 mean Chamfer L2: {np.nanmean(ch):.2f} mm   (baseline 22.78 mm)")
for d in (1, 2, 5):
    print(f"Ot@{d}mm: {np.mean(ov[d]):.3f}")
json.dump({"chamfer_per_case": dict(zip(map(str, va_ids), ch)),
          ...
          open(f"{OUT}/stage2_results_64.json", "w"), indent=2)
print("saved",
      f"{OUT}/stage2_results_64.json  and checkpoint at {CKPT}/vol_best_ema_64.pt")
